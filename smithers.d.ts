...
//
// WORKSPACE.ts and the PACKAGE.ts files are not part of the library build
// (the emit tsconfigs exclude them), so each pulls this file in with a
 ...
 * `S.NodeModule.Bin("typescript", "tsc")`, and `examples.srcs` on an
 * imported Package without constraining the design.
 */
